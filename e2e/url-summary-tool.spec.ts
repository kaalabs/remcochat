...
test("URL summary tool renders a card (WebKit)", async ({ page }) => {
  ...
  await createProfile(page, `E2E url summary ${Date.now()}`);
  ...
  await page.getByTestId("composer:textarea").fill(
    "Gebruik de displayUrlSummary tool met url=https://example.com en length=short."
  );
  await page.getByTestId("composer:submit").click();

  const card = page.getByTestId("tool:displayUrlSummary");
  await expect(card).toBeVisible({ timeout: 120_000 });
  await expect(card).toContainText(/Example Domain/i);
  await expect(card).toContainText(/example.com/i);
  await expect(card.getByRole("link", { name: /open link/i })).toBeVisible();
