...
test("Notes tool renders a card and deletes notes (WebKit)", async ({ page }) => {
  ...
  await createProfile(page, `E2E notes ${Date.now()}`);
  ...
  await page.getByTestId("composer:textarea").fill(
    "Note this: Bel de tandarts morgen om 10:00. Gebruik de displayNotes tool met action=create en content='Bel de tandarts morgen om 10:00'."
  );
  ...
  await expect(card).toContainText(/No notes yet/i);
