...
async function createProfile(request: import("@playwright/test").APIRequestContext) {
  const profileRes = await request.post("/api/profiles", {
    data: { name: `E2E skills script ${Date.now()}` },
  });
...
async function getToolsEnabledModelId(request: import("@playwright/test").APIRequestContext) {
  ...
  return (
    preferred.find((id) => toolEnabled.some((m) => m.id === id)) ??
    toolEnabled[0]?.id ??
    ""
  );
}
...
test("Skills script runs in Vercel Sandbox (optional)", async ({ request }) => {
  ...
  const temporarySessionId = `e2e-skills-script-${Date.now()}`;
          ...
          parts: [
            {
              type: "text",
              text: [
                "The sandbox project includes `./.skills/skills-system-validation/scripts/echo.sh`, which prints REMCOCHAT_SKILLS_SCRIPT_OK.",
                "Run the following command using the bash tool exactly as written, and do not add any other text:",
                "`bash ./.skills/skills-system-validation/scripts/echo.sh`",
              ].join("\n"),
            },
          ],
...
test("Skills script runs in Docker sandboxd (optional)", async ({ request }) => {
  test.skip(
  ...
  try {
    ...
    const profileId = await createProfile(request);
    const temporarySessionId = `e2e-skills-script-docker-${Date.now()}`;
